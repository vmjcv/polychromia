...
func with(data: Dictionary) -> float:
	...
	if not data.has('duration'):
		if _last_animation_duration > 0:
			...
			data.duration = UGC.autoload.get_autoload("Anima").DEFAULT_DURATION
...
func _setup_node_animation(data: Dictionary) -> float:
	var node = data.node
	var delay = data.delay if data.has('delay') else 0.0
	var duration = data.duration if data.has('duration') else UGC.autoload.get_autoload("Anima").DEFAULT_DURATION
	...
	if data.has('animation'):
		var script = UGC.autoload.get_autoload("Anima").get_animation_script(data.animation)
...
func _setup_grid_animation(animation_data: Dictionary) -> float:
	var animation_type = UGC.autoload.get_autoload("Anima").GRID.SEQUENCE_TOP_LEFT
	
	if animation_data.has('animation_type'):
		animation_type = animation_data.animation_type

	if not animation_data.has('items_delay'):
		animation_data.items_delay = UGC.autoload.get_autoload("Anima").DEFAULT_ITEMS_DELAY
	...
	var duration: float
	var Anima = UGC.autoload.get_autoload("Anima")
...
func _generate_animation_sequence(animation_data: Dictionary, start_from: int) -> float:
	...
	if start_from == UGC.autoload.get_autoload("Anima").GRID.FROM_POINT and not animation_data.has('point'):
		start_from = UGC.autoload.get_autoload("Anima").GRID.FROM_CENTER

	for row_index in children.size():
		var row: Array = children[row_index]
		var from_index = 0

		if start_from == UGC.autoload.get_autoload("Anima").GRID.SEQUENCE_BOTTOM_RIGHT:
			from_index = row.size() - 1
		elif start_from == UGC.autoload.get_autoload("Anima").GRID.FROM_CENTER:
			from_index = (row.size() - 1) / 2
		elif start_from == UGC.autoload.get_autoload("Anima").GRID.FROM_POINT:
			if is_grid:
