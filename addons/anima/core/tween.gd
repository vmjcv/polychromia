...
var _visibility_strategy: int = UGC.autoload.get_autoload("Anima").VISIBILITY.IGNORE
var _callbacks := {}
...
func add_animation_data(animation_data: Dictionary, play_mode: int = PLAY_MODE.NORMAL) -> void:
	...
	var duration = animation_data.duration if animation_data.has('duration') else UGC.autoload.get_autoload("Anima").DEFAULT_DURATION
...
func reset_data(strategy: int, play_mode: int, animation_length: float):
	...
	for animation_data in data:
		animation_data._recalculate_from_to = strategy == UGC.autoload.get_autoload("Anima").LOOP.RECALCULATE_RELATIVE_DATA and animation_data.has('relative')
...
func _apply_visibility_strategy(animation_data: Dictionary, strategy: int = UGC.autoload.get_autoload("Anima").VISIBILITY.IGNORE):
	...
	if strategy == UGC.autoload.get_autoload("Anima").VISIBILITY.HIDDEN_ONLY:
		should_hide_nodes = true
	elif strategy == UGC.autoload.get_autoload("Anima").VISIBILITY.HIDDEN_AND_TRANSPARENT:
		should_hide_nodes = true
		should_make_nodes_transparent = true
	elif strategy == UGC.autoload.get_autoload("Anima").VISIBILITY.TRANSPARENT_ONLY:
		should_make_nodes_transparent = true
...
func _on_tween_started(_ignore, key) -> void:
	...
	if hide_strategy == UGC.autoload.get_autoload("Anima").VISIBILITY.HIDDEN_ONLY:
		should_restore_visibility = true
	elif hide_strategy == UGC.autoload.get_autoload("Anima").VISIBILITY.HIDDEN_AND_TRANSPARENT:
		should_restore_modulate = true
		should_restore_visibility = true
	elif hide_strategy == UGC.autoload.get_autoload("Anima").VISIBILITY.TRANSPARENT_ONLY:
		should_restore_modulate = true
